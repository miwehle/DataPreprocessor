...
def plot_token_pair_length_histogram(
    ...
    de_lengths, en_lengths = load_token_pair_lengths(dataset_path)
    return base.plot_pair_length_histogram(
        de_lengths, en_lengths, "Histogram of Token Pair Lengths (de/en)",
        "Text length (tokens)", max_bins, x_scale, y_scale, interactive_scale_toggle,
    )
...
def run(dataset_path: str | Path) -> None:
    plot_token_pair_length_histogram(dataset_path)
    plt.show()
