...
def test_preprocess_calls_stages_in_order(monkeypatch):
    ...
    assert calls[-1][1]["output_path"] == run_dir / "artifacts" / "datasets" / "europarl" / "d1"
...
def test_preprocess_writes_snapshots_from_stage_names(monkeypatch):
    ...
    assert save_paths[0].parent.name == "d1_staging"
    assert save_paths[5] == run_dir / "artifacts" / "datasets" / "europarl" / "d1"
...
def test_preprocess_writes_dataset_manifest(monkeypatch):
    ...
    manifest_path = run_dir / "artifacts" / "datasets" / "europarl" / "d1" / "dataset_manifest.yaml"
...
def test_preprocess_uses_next_dataset_id(monkeypatch):
    ...
    (run_dir / "artifacts" / "datasets" / "europarl" / "d1").mkdir(parents=True, exist_ok=True)
    ...
    assert (run_dir / "artifacts" / "datasets" / "europarl" / "d2").is_dir()
    assert calls[-1][1]["output_path"] == run_dir / "artifacts" / "datasets" / "europarl" / "d2"
...
def test_preprocess_logs_to_dataset_preprocess_log(monkeypatch):
    ...
    log_path = run_dir / "artifacts" / "datasets" / "europarl" / "d1" / "preprocess.log"
...
def test_preprocess_uses_separate_staging_dir(monkeypatch):
    ...
    save_paths = [kwargs["output_path"] for name, kwargs in calls if name == "save"]
    assert save_paths[0] == staging_root / "europarl" / "d1_staging" / "europarl_load.jsonl"
    assert save_paths[-1] == run_dir / "artifacts" / "datasets" / "europarl" / "d1"
...
def test_preprocess_uses_dataset_name_for_generic_downloads(monkeypatch):
    ...
    assert save_paths[0].parent.name == "d1_staging"
    assert save_paths[4].parent.name == "d1_staging"
...
def test_preprocess_calls_split_after_writing_base_dataset(monkeypatch):
    ...
    assert split_call == api.SplitConfig(
        dataset=str(run_dir / "artifacts" / "datasets" / "europarl" / "d1"),
        split_ratio={"train": 0.9, "val": 0.1},
        seed=13,
    )
    config_text = (
        run_dir / "artifacts" / "datasets" / "europarl" / "d1" / "preprocess_config.yaml"
